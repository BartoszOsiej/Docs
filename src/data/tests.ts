...
export const projectTests: ProjectTests[] = [
  ...
  },
  {
    ...
    duration: '~0.01 s',
    status: pass,
    summary: 'eBPF endpoint security agent — 95/95 tests green, incl. the Ed25519 licensing module.',
    groups: [
      { title: 'Monitor core', count: 36, status: pass, coverage: 'C-string decoding, exec/open stats, ransomware heuristic, window expiry, stats sorting, network aggregation' },
      { title: 'Licensing', count: 30, status: pass, coverage: 'Ed25519 key verification, tiers, feature gating, expiry, machine binding, cache round-trip, key integrity' },
      { title: 'MeMLP engine', count: 17, status: pass, coverage: 'forward pass, online training, NaN/Inf sanitisation, checkpoint persistence' },
      { title: 'Audit log', count: 6, status: pass, coverage: 'hash chain end-to-end, write-and-verify, tamper detection' },
      { title: 'Sandbox', count: 6, status: pass, coverage: 'seccomp filter validity, capability checks, Landlock ABI' },
    ],
    checks: [
      { label: 'Clippy', result: '0 warnings with -D warnings', status: pass },
      { label: 'cargo fmt', result: 'clean', status: pass },
      { label: 'Licensing E2E', result: '18/18 HTTP scenarios green on the production activation server', status: pass },
    ],
    notes: ['The eBPF crate targets bpfel-unknown-none and is built via build.sh, not on the host toolchain.'],
    reportUrl: 'https://github.com/BartoszOsiej/talus-process-monitor/blob/master/TEST_REPORT_SECURE_LICENSING.md',
  },
  {
    id: 'externum',
    name: 'Externum',
